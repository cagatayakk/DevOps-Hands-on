...
metadata:
  name: prometheus
rules:
- apiGroups: [""]         # core sistem "apiversion=v1" olanlar icin gecerli
  resources:
  - nodes
  - nodes/proxy
  - services
...
- apiGroups:
  ...
  - ingresses
  verbs: ["get", "list", "watch"]
- nonResourceURLs: ["/metrics"]         # altinda dosya olmayanlar icin kullanilir
  verbs: ["get"]
---
apiVersion: rbac.authorization.k8s.io/v1
kind: ClusterRoleBinding      # role'ü service'e baglar
metadata:
  name: prometheus
roleRef:
  apiGroup: rbac.authorization.k8s.io
  kind: ClusterRole
  name: prometheus      # yukarida olusturulan role Serviceaccount'a baglamak icin kullanilir
subjects:
- kind: ServiceAccount
  name: default
  namespace: prometheus     # daha önceden olusturdugumuz namespace
```
