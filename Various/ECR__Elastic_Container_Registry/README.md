...
```bash
docker build -t todo-app:latest .
# docker build --force-rm -t todo-app:latest .        # uygulamada hata olursa son container kalir, onu silmek icin kullanilir.
```
- Tag the image to push it to ECR repository.
...
```bash                       
aws ecr batch-delete-image \              # repo icindeki image'lari silmek icin kullanilir
      --repository-name student2-repo/todo-app \
...
```bash
aws ecr delete-repository \          # repoyu silmek icin kullanilir    
      --repository-name student2-repo/todo-app \
      --force
```
