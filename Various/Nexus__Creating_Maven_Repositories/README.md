...
```
sudo sed -i s/\$releasever/6/g /etc/yum.repos.d/epel-apache-maven.repo      # dosyayi bularak icinde degisiklik yapmak icin kullanilir
```
...
```
cd /opt
sudo wget -O nexus.tar.gz https://download.sonatype.com/nexus/3/latest-unix.tar.gz    # "-O" inen dosyaya istenilen ismi vermek icin kullanilir
ls
```
...
```
ll
sudo chown -R ec2-user:ec2-user /opt/nexus            # ec2-user kullanicisina ve grubuna ekledik. (chown - change owner)
sudo chown -R ec2-user:ec2-user /opt/sonatype-work    ## ec2-user kullanicisina ve grubuna ekledik
ll
```
...
```
mvn
curl http://169.254.169.254/latest/meta-data/public-ipv4    # private-ip kullanimi daha uygundur
