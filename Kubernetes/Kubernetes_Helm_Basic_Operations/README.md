...
- Learn how to use AWS EC2 instance and Github as helm repo
...
tar -xvf mysql-*.tgz        # dosyayi ayiklamak ve klasöre cikarmak icin kullanilir
```
...
```bash
helm install --debug --dry-run mydryrun clarus-chart    # "--debug --dry-run"komutu bu chart calisirsa ne olacak onu görmek icin kullanilir. Terraform plan mantiginda calisir
```
...
```bash
helm install --debug --dry-run setflag clarus-chart --set course=AWS    # value degerinin üzerine yazmak icin kullanilir
```
