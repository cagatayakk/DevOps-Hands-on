...
```bash
ping -c 4 clarus2nd     # cd /etc && vi hosts  and 172.17.0.3 clarus2nd
```
...
```bash
docker container stop clarus1st clarus2nd       # docker container stop $(docker ps -a -q)   stop all container
docker container rm clarus1st clarus2nd         # docker container rm $(docker ps -a -q)  delete al container
```
...
```bash
docker container run --rm -d -p 8080:80 --name ng nginx     # "--rm" container durunca otomatik olarak sil
```
...
```bash
docker container exec -it my_nginx sh       #   "exec" komutu ciktigin zaman calismaya devam etmesi icin
apt-get update
