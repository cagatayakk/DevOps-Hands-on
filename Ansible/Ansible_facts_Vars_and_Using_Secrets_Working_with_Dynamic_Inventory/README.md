...
```bash
$ ansible node1 -m setup    # node hakkinda bilgi almak icin kullanilir
```
```
...
- name: show facts
  ...
  tasks:
    - name: print facts
      debug:
        var: ansible_facts      # node hakkinda bilgi almak icin kullanilir
```
- run the play book
...
- hosts: all
  ...
  - name: show IP address
    debug:
      msg: >      # istenilen formatta ve sadece istenilen bilgiyi yazdirmak icin kullanilir
...
ansible-vault decrypt secret.yaml       # sifreli icerigi gormek icin kullanilir
ansible-vault encrypt secret.yaml       # sifresi acilmis icerigi tekrar sifrelemek icin kullanilir
```
```
...
```
- Run the playbook with "--ask-vault-pass" command:     # sifreli dosyanin sifresi girilerek acilir
...
keyed_groups:
  - key: tags.Name
compose:              # degisken olarak tanimlayarak bilgi almak icin kullanilir.
  ansible_host: public_ip_address   # ansible_host artik degisken ismi
...
- name: create a user using a variable
  ...
  become: true
  vars:
    user: lisa      # olusturulacak kullanici degisken olarak atandi
    ansible_ssh_private_key_file: "/home/ec2-user/<pem file>"
  tasks:
