...
$ vim hosts
[webservers]
node1 ansible_host=<node1_ip> ansible_user=ec2-user         # private ec2 ip verilebilir
...
```bash
$ scp -i <pem file> <gönderilecek dosya> ec2-user@<public DNS name of Control Node>:/home/ec2-user      # güvenli bir sekilde dosya kopyalamak icin kullanilir
```
...
```bash
$ ansible-doc ping    # ping hakkindaki dökümantasyonu consoldan görmek icin kullanilir
```
...
```bash
$ ansible all -m ping -o    # "-o" tüm ciktiyi tek bir satirda almak icin kullanilir
```
...
```
[defaults]
interpreter_python=auto_silent    # python hakkinda cikan uyariyi kapatmak icin kullanilir
```
...
```bash
$ ansible webservers -m shell -a "systemctl status sshd"    # diger nodlara baglanir ve sondaki komutu calistirir ve ciktisini döndürür
```
- Explain the output.
...
$ ansible node1 -m shell -a "echo Hello Clarusway > /home/ec2-user/testfile2 ; cat testfile2 ; ls"
```
...
$ ansible all -m shell -a "echo Neler oluyor hayatta > ~/test4"
```
...
```bash
ansible webservers -b -m shell -a "amazon-linux-extras install -y nginx1 ; systemctl start nginx ; systemctl enable nginx"       # "-b" sudo yetkisi vermek icin kullanilir
ansible node1 -b -m shell -a "echo '<h1> hello clarusway</h1>' > /usr/share/nginx/html/index.html"
```
...
```bash
$ ansible webservers -b -m yum -a "name=nginx state=present"      # "yum" amazon linux makine icin kullanilir
$ ansible node3 -b -m apt -a "name=nginx state=present"           # "apt" ubuntu makine icin kullanilir
```
...
```bash
$ ansible -b -m package -a "name=nginx state=present" all       # hem ubuntu hem amazon linux makineler icin kullanilabilir
```
...
```bash
$ vim inventory     # isim farketmeksizin kendi inventory dosyamizi olusturabiliriz
```
```bash
...
```bash
ansible -i inventory -b -m yum -a "name=httpd state=present" node1    # "present veya latest" yükleme yapar
ansible -i inventory -b -m yum -a "name=httpd state=absent" node1     # "absent" silme yapar
```
