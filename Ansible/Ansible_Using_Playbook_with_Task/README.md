...
# asagidaki kullanimda hepsi icin gecerli olacak sekilde kullanilabilir
# yukaridaki komutlar ile ayni islevi yapar

- name : Copy for all
  hosts: all
  tasks:
    - name: Copy yor file to the all servers
      copy:
        src: ~/testfile1
        dest: ~/testfile1
```
...
---                         # bu sekilde de yükleme yapilabilir
    #  yum:
    #     name:
    #       - httpd               
    #       - nginx
  ...
  tasks:
   - name: installing apache
     apt:
       name: apache2
       state: latest
     become: yes      # root yetkisi vermek icin kullanilir
...
- name: play 5
  ...
  tasks:
    - name: installing httpd and wget
      yum:
        pkg: "{{ item }}"   # /name: "{{ item }}"/ olarak da kullanilabilir
        state: present
      loop:
        - httpd
        - wget
...
```bash
ansible-playbook playbook6.yml     # (become:yes) oldugu icin burada "-b" belirtmemize gerek yok
```
  ...
  tasks:
   - name: Uninstalling Apache
     apt:
       name: apache2
       state: absent
       update_cache: yes     # apache kaldirildiktan sonra cache temizlemek icin
   - name: Remove unwanted Apache2 packages
     apt:
       autoremove: yes    # dependency dosyalarini kaldirir
       purge: yes     # temizlik yapar
...
---
- name: Create users
  hosts: "*"        # bütün hostlar icin yap anlamindadir
  tasks:
    - user:
        name: "{{ item }}"
        state: present
